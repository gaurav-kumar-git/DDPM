...
def sampleConditional(model, n_samples, noise_scheduler, class_label):
    """
    Generates samples from a trained conditional diffusion model.
    
    Implements Algorithm 2 from DDPM (Ho et al., 2020), conditioning on a specified class.
    
    Args:
        model (nn.Module): The trained conditional diffusion model.
        n_samples (int): Number of samples to generate.
        noise_scheduler (NoiseScheduler): Scheduler managing noise parameters.
        class_label (int): The class label used for conditioning.
    ...
    return x_t  # Return final generated samples
...
if __name__ == "__main__":
    ...
    if args.type == 'conditional':
        ...
        if args.mode == 'train':
            ...
            model.load_state_dict(torch.load(f'{run_name}/conditional_model.pth'))
            ...
            for class_label in unique_classes:
                samples, sample_labels = sampleConditional(model, n_samples_per_class, noise_scheduler, class_labels=class_label.item())
                all_samples.append(samples)
                all_labels.append(sample_labels)

            # Concatenate all generated samples and labels
            all_samples = torch.cat(all_samples, dim=0)
            all_labels = torch.cat(all_labels, dim=0)

            # Save the generated samples along with their labels
            torch.save({'samples': all_samples, 'labels': all_labels}, f'{run_name}/samples_{args.seed}_{args.n_samples}.pth')   
    else:
